...
$('.js-result').on('click keyup', function() {
...
$('.cart-checkbox-all').change(function() {
	...
	$subTotalContainer = $('#cartAccordion').find('.woocommerce-Price-amount');
	if ($(this).is(':checked')) {
		$('.cart-checkbox').each(function(idx,elem) {
			$(elem).prop("checked", true);
			data.push({ cart_no: $(elem).data('cart_no') })
		})
	} else {
		$('.cart-checkbox').each(function(idx,elem) {
...
/* 개별 체크박스 선택에 따른 All checkbox 변경함수 */
function checkSelectAll(){
	//전체 체크박스
	const Allcheckboxes = document.querySelectorAll('input[name="cartItem"]');
	//선택된 체크박스
	const checkedBox = document.querySelectorAll('input[name="cartItem"]:checked');
	//select All체크박스
	const selectAll = document.querySelector('input[name="selectAll"]');
	
	if(Allcheckboxes.length === checkedBox.length) selectAll.checked = true;
