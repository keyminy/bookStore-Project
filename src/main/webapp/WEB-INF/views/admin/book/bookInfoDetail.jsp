                ...
                <form method="post" action="/admin/bookUpdate" enctype="multipart/form-data" onsubmit="return checkSubmit()">
                   ...
                   <div class="mb-3">
                    	...
                    	<select name="itemSellStatus" style="margin-left: 10px;">
                    		<option value="SELL" <c:if test ="${bookInfo.itemSellStatus eq 'SELL'}">selected</c:if>>판매 중</option>
                    		<option value="SOLD_OUT" <c:if test ="${bookInfo.itemSellStatus eq 'SOLD_OUT'}">selected</c:if>>품절</option>
                    	</select>
                   </div>
                    <button type="submit" class="btn btn-outline-primary" style="float: left;">책 정보 수정</button>
